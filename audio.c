...
unsigned char AudioReadFromReg(unsigned char u8RegAddr) {
    unsigned char u8TxData[2];
    unsigned char u8RxData[1];
    int Status;

    // Set the register address to read from
    u8TxData[0] = 0x40; // Register write address [15:8]
    u8TxData[1] = u8RegAddr; // Register write address [7:0]

    // Send the register address to the ADAU1761
    Status = XIicPs_MasterSendPolled(&Iic, u8TxData, 2, (IIC_SLAVE_ADDR >> 1));
    if (Status != XST_SUCCESS) {
        xil_printf("Error sending register address: %02x\n", u8RegAddr);
        return 0xFF; // Return an error value
    }
    while (XIicPs_BusIsBusy(&Iic));

    // Read the register value from the ADAU1761
    Status = XIicPs_MasterRecvPolled(&Iic, u8RxData, 1, (IIC_SLAVE_ADDR >> 1));
    if (Status != XST_SUCCESS) {
        xil_printf("Error reading register value: %02x\n", u8RegAddr);
        return 0xFF; // Return an error value
    }
    while (XIicPs_BusIsBusy(&Iic));
...
void VerifyAudioConfiguration() {
    // Verify PLL Lock
    unsigned char pllStatus = AudioReadFromReg(R1_PLL_CONTROL);
    xil_printf("PLL Control Register (R1): %02x\n", pllStatus);

    // Verify ADC Control
    unsigned char adcControl = AudioReadFromReg(R19_ADC_CONTROL);
    xil_printf("ADC Control Register (R19): %02x\n", adcControl);

    // Verify DAC Control
    unsigned char dacControl = AudioReadFromReg(R36_DAC_CONTROL_0);
    xil_printf("DAC Control Register (R36): %02x\n", dacControl);

    // Verify Playback Power Management
    unsigned char playbackPower = AudioReadFromReg(R35_PLAYBACK_POWER_MANAGEMENT);
    xil_printf("Playback Power Management Register (R35): %02x\n", playbackPower);

    // Verify Volume Settings
    unsigned char leftVolume = AudioReadFromReg(R29_PLAYBACK_HEADPHONE_LEFT_VOLUME_CONTROL);
    unsigned char rightVolume = AudioReadFromReg(R30_PLAYBACK_HEADPHONE_RIGHT_VOLUME_CONTROL);
    xil_printf("Left Headphone Volume (R29): %02x\n", leftVolume);
    xil_printf("Right Headphone Volume (R30): %02x\n", rightVolume);
}
